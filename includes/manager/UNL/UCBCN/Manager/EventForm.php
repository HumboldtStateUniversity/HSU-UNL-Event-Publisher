...
class UNL_UCBCN_Manager_EventForm
{
    ...
    function getRelatedLocationDateAndTimes(UNL_UCBCN_Event $event)
    {
        ...
        if ($instances) {
            ...
            if (!isset($_REQUEST['rec']) && !isset($_REQUEST['recid'])) {
                $table->addRow(array('<a class="subsectionlink" href="'.$this->manager->uri.'?action=eventdatetime&event_id='.$event->id.'">Add additional location, date and time.</a>'), 'colspan=6');
            }
